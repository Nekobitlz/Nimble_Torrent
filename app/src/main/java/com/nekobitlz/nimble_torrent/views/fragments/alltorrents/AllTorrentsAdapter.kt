...
class AllTorrentsAdapter(val onDeleteClick: (View, Int) -> Unit) : RecyclerView.Adapter<AllTorrentsViewHolder>() {

    var torrentFiles: List<TorrentData> = mutableListOf()

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): AllTorrentsViewHolder {
        val itemView =
            LayoutInflater.from(parent.context).inflate(R.layout.item_download, parent, false)

        return AllTorrentsViewHolder(itemView)
    }

    override fun getItemCount(): Int = torrentFiles.size

    override fun onBindViewHolder(holder: AllTorrentsViewHolder, position: Int) {
        holder.bind(torrentFiles[position])

        holder.onLongClick(holder.itemView, position) { view, viewPosition ->
            ...
            popup.setOnMenuItemClickListener {
                when (it.itemId) {
                    R.id.menu_item_delete -> onDeleteClick(view, viewPosition)
                }
                true
            }
            popup.show()
...
class AllTorrentsViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
